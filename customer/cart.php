...
if(!isset($_SESSION['cart'])){
	// CHưa có giỏ hàng -> tạo giỏ
	$_SESSION = array();
}
session_start();
// Kiểm tra xem người dùng có thểm sp hay ko ?
if(isset($_GET['action']) && $_GET['action']='add'){
	...
	// CHuyển hướng lại chính trang giỏ hàng
...
<body>
		...
		<!-- your cart -->
		<div style="height: 100px; width: auto;text-align: center;border-bottom: 1px solid #efefed; padding-top: 20px;">
			...
			<?php
				...
				if(isset($_SESSION['cart'])){
					foreach($_SESSION['cart'] as $id_product => $quantity ){
						// Truy vấn và lấy thông tin từng sản phẩm 1
						...
						echo "<tr>";
							...
							// so luong 
							echo "<td>";
								echo "<a href='update.php?id=$id_product&action=giamsl' style='font-size:19px;'>-</a>";
								echo $quantity;
								echo "<a href='update.php?id=$id_product&action=tangsl' style='padding-left:5px; font-size:16px;'>+</a>";
							echo "</td>";
							$total_amount += $row['price'] * $quantity;
							echo "<td>"."$".($row['price'] * $quantity).".00"."</td>";
							echo "<td>";
								echo "<a style='font-family:monospace' href='deleteCartItem.php?id=$id_product'>X</a>";
							echo "</td>";
						echo "</tr>";
					}
					...
					echo "<h2 style='text-align:center;font-family:monospace ;padding-top:9px;'>Empty Cart</h2>";
				}
			?>
			
			</table>
			
				<a style="float:right;padding-right: 100px;" href="checkout.php">
					Check Out
				</a>
		</div>
